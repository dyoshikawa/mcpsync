...
export async function importCommand(options: ImportOptions): Promise<void> {
  ...
  const toolsToImport = Object.entries(options).filter(
    ([key, value]) => value === true && key !== "verbose"
  ...
  const [toolName] = toolsToImport[0] as [keyof typeof CONFIG_PATHS, boolean];
  const configPath = findConfigFile(toolName);

  if (!configPath) {
    console.error(`No MCP configuration found for ${toolName}`);
    if (options.verbose) {
      console.log(`Searched paths:`, CONFIG_PATHS[toolName]);
    }
    return;
  }

  const importedConfig = readMcpConfig(configPath, toolName);
  if (!importedConfig) {
    console.error(`Failed to read MCP configuration from ${configPath}`);
    return;
  }

  const mergedConfig = mergeConfigs(existingConfig, importedConfig, toolName);

  // Write merged config to mcp.json
  try {
    writeFileSync(mcpJsonPath, JSON.stringify(mergedConfig, null, 2));
    console.log(`Successfully imported MCP configuration from ${toolName}`);
